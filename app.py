...
def byurl():
    if request.method == 'POST':
        ...
        try:
            ...
            os.remove(filename)
            return jsonify({"filename": "invalid file", "NoofPerson": 0})
        count = open_read_image(filename)
        gc.collect()
        return json.dumps({"filename": filename, "NoofPerson": count})
    return jsonify({"filename": "invalid file", "NoofPerson": 0})
    ...
    if request.method == "POST":
        ...
        if request.files:
            ...
            if allowed_image(app, image.filename):
                ...
                count = open_read_image(image.filename)
                return json.dumps({"filename": image.filename, "NoofPerson": count})
            else:
                print("That file extension is not allowed")
                return json.dumps({"filename": "invalid file", "NoofPerson": 0})
        return jsonify({"filename": "invalid file", "NoofPerson": 0})
